...
import { AuthenticatedRequest, RefreshTokenPayload, MulterFiles } from "../types/index.js";
...
const refreshAccessToken = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    return res
      .status(401)
      .json(
        new ApiResponse(401, null, err?.message || "Invalid Refresh Token")
      );
  }
});
...
const getCurrentUser = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  return res
    .status(200)
    .json(new ApiResponse(200, req.user, "Current user fetched Succes"));
});
...
const getUserWatchHistory = asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const userWatchHistory = await User.aggregate([
    {
      $match: {
        _id: new mongoose.Types.ObjectId(req.user?._id as unknown as string),
      },
      $lookup: {
        from: "videos",
