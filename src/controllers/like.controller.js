...
const toggleVideoLike = asyncHandler(async (req, res) => {
  try {
    ...
    if (isLiked) {
      ...
      res
        .status(200)
        .json(new ApiResponse(200, "Toggled video Like successfully"));
    } else {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          500,
          null,
          `Failed to toggle Video Like due to ${error.message}`
        )
      );
  }
});
...
const toggleCommentLike = asyncHandler(async (req, res) => {
  try {
    ...
    if (isLiked) {
      ...
      res
        .status(200)
        .json(new ApiResponse(200, "Toggle Comment Like Succesfully"));
    } else {
    ...
    return res
      ...
      .json(
        new ApiResponse(
          500,
          null,
          `Failed to toggle Comment Like due to ${error.message}`
        )
      );
  }
});
    ...
    return res
      ...
      .json(
        new ApiResponse(
          500,
          null,
          `Failed to toggle Tweet Like due to ${error.message}`
        )
      );
  }
});
    ...
    return res
      ...
      .json(
        new ApiResponse(
          500,
          null,
          `Failed to get liked Videos due to ${error.message}`
        )
      );
  }
});
